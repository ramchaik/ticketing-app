...
it('fetches orders for a paritcular user', async () => {
  // Create 3 tickets
  ...
  expect(ordersResponse.body[0].ticket.id).toEqual(ticketTwo.id);
  expect(ordersResponse.body[1].ticket.id).toEqual(ticketThree.id);
